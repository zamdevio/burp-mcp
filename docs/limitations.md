...
- Burp often uses a **shared** request/response editor for Repeater. Reading another tab may require selecting it briefly; incomplete settle/restore logic can return the wrong body or leave selection changed.
...
Planned hardening (enforce select/restore, Send tools, more Montoya coverage) is tracked under [`maintainer/phases/`](../maintainer/phases/) — not promised by this page until shipped and listed in [`maintainer/shipped/`](../maintainer/shipped/).
